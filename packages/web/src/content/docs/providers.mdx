...
2. Select **SuperGrok Subscription**. Oimo opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Oimo. Oimo polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Oimo refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
