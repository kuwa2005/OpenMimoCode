...
Every Open Mimo Code server already serves an OpenAI-compatible surface over the models this
project has configured, mounted at `/v1`. There is no separate LLM server to start and no
daemon to manage: if a Open Mimo Code process is listening, the endpoints are there.
...
Open Mimo Code **defines** this interface (llm / asr / tts / voice design / voice clone); consumers
adapt to it.
...
`oimo llm-server issue` can print a base URL. A spawned child should be **told** its
endpoint by whoever spawned it rather than picking one out of that directory.

## Minting a token

`oimo llm-server` only mints and manages credentials; it starts nothing.

```bash
oimo llm-server issue --capability transcription --json --label my-skill
oimo llm-server issue --model anthropic/claude-haiku-4-5 --ttl 12h
oimo llm-server list [--json]
oimo llm-server revoke <id> | --all
```
...
- `--json` prints `api_key`, `id`, `base_url`, `expires_at`, `models`, and — with
  ...
  this installation, so a consumer can recover from expiry without knowing how Open Mimo Code was
...
| 502 | — | The upstream provider failed, not Open Mimo Code |
