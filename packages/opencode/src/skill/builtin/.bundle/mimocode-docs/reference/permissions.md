# Open Mimo Code Permissions Reference
...
`mcp_sampling` gates MCP **client-side sampling**: an MCP server asking Open Mimo Code to run a model call on its behalf (`sampling/createMessage`), so the server needs no API key of its own. The glob argument is the MCP server name, so you can allow one server and keep the rest at `ask`:
...
| Surface | How |
|---------|-----|
| TUI (`oimo`) | `oimo --dangerously-skip-permissions` |
| TUI at runtime | `/skip-permissions` — toggle mid-session, instance-wide, inherited by subagents |
| Headless (`oimo run`) | `oimo run --dangerously-skip-permissions "<prompt>"` |
...
- A top-level catch-all like `"permission": { "*": "ask" }` makes the TUI/env form a no-op (your `*: ask` outranks the injected `*: allow`). Remove it, or use `oimo run --dangerously-skip-permissions`, which auto-replies at the event layer and overrides `ask` too.
...
- `external_directory` governs reads/writes outside the project working directory — by default these prompt, so Open Mimo Code never silently widens scope.
