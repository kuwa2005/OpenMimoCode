...
| Surface | How |
|---------|-----|
| TUI (`omimo`) | `omimo --dangerously-skip-permissions` |
| TUI at runtime | `/skip-permissions` — toggle mid-session, instance-wide, inherited by subagents |
| Headless (`omimo run`) | `omimo run --dangerously-skip-permissions "<prompt>"` |
...
- A top-level catch-all like `"permission": { "*": "ask" }` makes the TUI/env form a no-op (your `*: ask` outranks the injected `*: allow`). Remove it, or use `omimo run --dangerously-skip-permissions`, which auto-replies at the event layer and overrides `ask` too.
