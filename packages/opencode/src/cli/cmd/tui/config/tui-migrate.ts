...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from oimo.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const oimo = await oimoFiles(input)
  for (const file of oimo) {
...
async function oimoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "oimo"),
    ...(await Filesystem.findUp(["oimo.json", "oimo.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "oimo"))
  }
