...
export const dict = {
  ...
  "tui.tips.run": "用 {highlight}omimo run{/highlight} 进行非交互式脚本调用",
  "tui.tips.continue": "用 {highlight}omimo --continue{/highlight} 继续上一个会话",
  "tui.tips.attach_cli": "用 {highlight}omimo run -f file.ts{/highlight} 通过命令行附加文件",
  "tui.tips.format_json": "用 {highlight}--format json{/highlight} 在脚本中获得机器可读输出",
  "tui.tips.serve": "运行 {highlight}omimo serve{/highlight} 以无头模式提供 MiMoCode API",
  "tui.tips.attach_server": "用 {highlight}omimo run --attach{/highlight} 接入正在运行的服务",
  "tui.tips.upgrade": "运行 {highlight}omimo upgrade{/highlight} 升级到最新版本",
  "tui.tips.auth_list": "运行 {highlight}omimo auth list{/highlight} 查看所有已配置的 provider",
  "tui.tips.agent_create": "运行 {highlight}omimo agent create{/highlight} 引导式创建智能体",
  "tui.tips.github_install": "运行 {highlight}omimo github install{/highlight} 配置 GitHub workflow",
  ...
  "tui.tips.debug_config": "运行 {highlight}omimo debug config{/highlight} 排查配置问题",
  ...
  "cli.providers.mimo_free.usage_hint": "无需登录，直接 omimo run 即可使用。如需付费/更高级模型，可重新选择 MiMo 浏览器登录。",
